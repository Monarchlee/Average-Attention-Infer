...
if __name__ == "__main__":
    ...
    parser.add_argument('--mv_set', type=int, default=24)
    parser.add_argument('--pkl_path', type=str,
                        default="../results/batch_pkl_output/mmlu_preorder_validation_origin_0shot_results_0_299_nosymb_0temp.pkl",
    ...
    if args.mv_set == 4:
        ...
        raise NotImplementedError('such mv_set is not supported yet')

    # load forward
    with open(args.pkl_path, 'rb') as f:
        debiased_res = pickle.load(f)
    ...
    for count in tqdm(range(1, data_len + 1, batch_size), desc="Processing"):
        ...
        for i in mv_set:
            ...
            for j in range(choice_num):
                # the debiased_res prob is in special order
                probs[debiased_res[count - 1]['choices'].index(debiased_res[idx]['choices'][j])] += debiased_res[idx]['probs'][j]
        probs = [p/len(mv_set) for p in probs]
        mv_ans = torch.argmax(torch.tensor(probs)).item() # should be an int
        ...
        # anyway, the mv_ans is in original order as mv should be done in a batch
        batch_ans_order = [mv_ans]*batch_size 
        batch_ans = []
        ...
        if len(num_vec) < choice_num: # there may same option in dirty dataset
            new_append = [0]*(choice_num - len(num_vec))
            num_vec.extend(new_append)
