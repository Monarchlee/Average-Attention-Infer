...
if __name__ == "__main__":
    ...
    parser.add_argument('--cmp_path', type=str,
                        default="../results/pkl_output/cosmosqa_preorder_validation_origin_0shot_results_0_299_nosymb_0temp.pkl",
                        help="the compared result only enable when debias method is not No")
    # parser.add_argument('--cmp_path', type=str, default="../results/pkl_output/cosmosqa_preorder_validation_origin_qwen_14b_chat_0shot_results_0_299_nosymb_0temp.pkl",
    #                   help="the compared result only enable when debias method is not No")
    ...
    for count in tqdm(range(1, data_len + 1, batch_size), desc="Processing"):
        ...
        if cmp_file is not None:  # just load the former res
            old_ans = [cmp_file[idx]['answer'] for idx in range(count-1,count-1+batch_size)]   # should be a list of [bsz, 4]
        else:
            raise NotImplementedError('no cmp_file provided')

        question_txt = permutation_data['question_expand'][count - 1]
        batch_oldans = old_ans  # should be list of [bsz]

        batch_text_ans = answer_text_batch
        num_ans = [filter_ans_nosymb(answer_text_batch[i], batch_choices[i]) for i in range(batch_size)]
        ...
        for bt in range(batch_size):
            ...
            question_answer_pair['answer'] = num_ans[bt]
            question_answer_pair['answer_text'] = batch_text_ans[bt]
            question_answer_pair['gt_ans'] = permutation_data['choices'][idx].index(permutation_data['gt_ans'][idx])
        ...
        for x in range(batch_size):
            if num_ans[x] != batch_oldans[x]:
                diff_count += 1
                total_diff_count += 1
        print(f"\nAnswer: {batch_text_ans[0]}")
    ...
    final_res['exclusion_acc'] = expand_acc / ((end - begin) * batch_size - invalid_num)
